...
namespace REV::Math
{
    union REV_INTRIN_TYPE REV_ALIGN(16) xmm
    {
        __m128  f;
        __m128i i;
        __m128d d;
    ...
    union REV_INTRIN_TYPE REV_ALIGN(32) ymm
    {
        __m256  f;
        __m256i i;
        __m256d d;
    ...
    union REV_INTRIN_TYPE REV_ALIGN(64) zmm
    {
        __m512  f;
        __m512i i;
        __m512d d;
    ...
    REV_INLINE b32 __vectorcall mm_equals(T *a, U *b)
    {
    #if REV_ISA >= REV_ISA_AVX512
    ...
    template<typename T, typename U>
    REV_INLINE b32 __vectorcall mm256_equals(T *a, U *b)
    {
    #if REV_ISA >= REV_ISA_AVX512
    ...
    template<typename T, typename U>
    REV_INLINE b32 __vectorcall mm512_equals(T *a, U *b)
    {
    #if REV_ISA >= REV_ISA_AVX512
    ...
    template<u8 index> REV_INLINE f32 __vectorcall mm_extract_f32(__m128  mm) { return reg32(_mm_extract_ps(mm, index)).f; }
    template<u8 index> REV_INLINE s32 __vectorcall mm_extract_s32(__m128i mm) { return _mm_extract_epi32(mm, index); }
    template<u8 index> REV_INLINE u32 __vectorcall mm_extract_u32(__m128i mm) { return cast<u32>(_mm_extract_epi32(mm, index)); }

    template<> REV_INLINE f32 __vectorcall mm_extract_f32<0>(__m128  mm) { return _mm_cvtss_f32(mm); }
    template<> REV_INLINE s32 __vectorcall mm_extract_s32<0>(__m128i mm) { return _mm_cvtsi128_si32(mm); }
    template<> REV_INLINE u32 __vectorcall mm_extract_u32<0>(__m128i mm) { return cast<u32>(_mm_cvtsi128_si32(mm)); }

    #if REV_ISA >= REV_ISA_AVX
        template<u8 index> REV_INLINE f32 __vectorcall mm_extract_f32(__m256  mm) { return reg32(_mm256_extract_epi32(ymm(mm).i, index)).f; }
        template<u8 index> REV_INLINE s32 __vectorcall mm_extract_s32(__m256i mm) { return _mm256_extract_epi32(mm, index); }
        template<u8 index> REV_INLINE u32 __vectorcall mm_extract_u32(__m256i mm) { return cast<u32>(_mm256_extract_epi32(mm, index)); }

        template<> REV_INLINE f32 __vectorcall mm_extract_f32<0>(__m256  mm) { return _mm_cvtss_f32(_mm256_castps256_ps128(mm)); }
        template<> REV_INLINE s32 __vectorcall mm_extract_s32<0>(__m256i mm) { return _mm_cvtsi128_si32(_mm256_castsi256_si128(mm)); }
        template<> REV_INLINE u32 __vectorcall mm_extract_u32<0>(__m256i mm) { return cast<u32>(_mm_cvtsi128_si32(_mm256_castsi256_si128(mm))); }
    #endif

    #if REV_ISA >= REV_ISA_AVX512
        template<u8 index> REV_INLINE f32 __vectorcall mm_extract_f32(__m512  mm) { return reg32(_mm_extract_ps(_mm512_extractf32x4_ps(mm, index / 4), index % 4)).f; }
        template<u8 index> REV_INLINE s32 __vectorcall mm_extract_s32(__m512i mm) { return _mm_extract_epi32(_mm512_extracti32x4_epi32(mm, index / 4), index % 4); }
        template<u8 index> REV_INLINE u32 __vectorcall mm_extract_u32(__m512i mm) { return cast<u32>(_mm_extract_epi32(_mm512_extracti32x4_epi32(mm, index / 4), index % 4)); }

        template<> REV_INLINE f32 __vectorcall mm_extract_f32<0>(__m512  mm) { return _mm512_cvtss_f32(mm); }
        template<> REV_INLINE s32 __vectorcall mm_extract_s32<0>(__m512i mm) { return _mm512_cvtsi512_si32(mm); }
        template<> REV_INLINE u32 __vectorcall mm_extract_u32<0>(__m512i mm) { return cast<u32>(_mm512_cvtsi512_si32(mm)); }
    #endif

    template<u8 index> REV_INLINE __m128  __vectorcall mm_insert_f32(__m128  mm, f32 val) { return xmm(_mm_insert_epi32(xmm(mm).i, reg32(val).i, index)).f; }
    template<u8 index> REV_INLINE __m128i __vectorcall mm_insert_s32(__m128i mm, s32 val) { return _mm_insert_epi32(mm, val, index); }
    template<u8 index> REV_INLINE __m128i __vectorcall mm_insert_u32(__m128i mm, u32 val) { return _mm_insert_epi32(mm, val, index); }

    #if REV_ISA >= REV_ISA_AVX
        template<u8 index> REV_INLINE __m256  __vectorcall mm_insert_f32(__m256  mm, f32 val) { return ymm(_mm256_insert_epi32(ymm(mm).i, reg32(val).i, index)).f; }
        template<u8 index> REV_INLINE __m256i __vectorcall mm_insert_s32(__m256i mm, s32 val) { return _mm256_insert_epi32(mm, val, index); }
        template<u8 index> REV_INLINE __m256i __vectorcall mm_insert_u32(__m256i mm, u32 val) { return _mm256_insert_epi32(mm, val, index); }
    #endif

    #if REV_ISA >= REV_ISA_AVX512
        template<u8 index>
        REV_INLINE __m512 __vectorcall mm_insert_f32(__m512 mm, f32 val)
        {
        ...
        template<u8 index>
        REV_INLINE __m512i __vectorcall mm_insert_s32(__m512i mm, s32 val)
        {
        ...
        template<u8 index>
        REV_INLINE __m512i __vectorcall mm_insert_u32(__m512i mm, u32 val)
        {
            __m128i paste = _mm512_extracti32x4_epi32(mm, index / 4);
            paste = _mm_insert_epi32(paste, val, index % 4);
            return _mm512_inserti32x4(mm, paste, index / 4);
