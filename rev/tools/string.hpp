...
public:
    ...
    explicit String(Allocator_t *allocator, u64 initial_capacity = 16, u64 alignment_in_bytes = REV::DEFAULT_ALIGNMENT)
        : m_Header(null)
    {
        REV_CHECK(allocator);
    ...
    String(Allocator_t *allocator, char symbol, u64 count, u64 alignment_in_bytes = REV::DEFAULT_ALIGNMENT)
        : m_Header(null)
    {
        REV_CHECK(allocator);
        if (alignment_in_bytes < REV::DEFAULT_ALIGNMENT) alignment_in_bytes = REV::DEFAULT_ALIGNMENT;
    ...
    String(Allocator_t *allocator, const char *cstring, u64 length, u64 alignment_in_bytes = REV::DEFAULT_ALIGNMENT)
        : m_Header(null)
    {
        REV_CHECK(allocator);
        if (alignment_in_bytes < REV::DEFAULT_ALIGNMENT) alignment_in_bytes = REV::DEFAULT_ALIGNMENT;
    ...
    String(Allocator_t *allocator, const ConstString& const_string, u64 alignment_in_bytes = REV::DEFAULT_ALIGNMENT)
        : m_Header(null)
    {
        REV_CHECK(allocator);
        if (alignment_in_bytes < REV::DEFAULT_ALIGNMENT) alignment_in_bytes = REV::DEFAULT_ALIGNMENT;
    ...
    String(Allocator_t *allocator, const StaticString<ss_capacity, ss_aligned_capacity>& static_string, u64 alignment_in_bytes = REV::DEFAULT_ALIGNMENT)
        : m_Header(null)
    {
        REV_CHECK(allocator);
        if (alignment_in_bytes < REV::DEFAULT_ALIGNMENT) alignment_in_bytes = REV::DEFAULT_ALIGNMENT;
...
template<typename LAllocator>                                           REV_INLINE bool operator==(const String<LAllocator>& left, const char                                           *right) { return left.Length() == strlen(right)  && !left.Compare(right); }
template<typename LAllocator>                                           REV_INLINE bool operator==(const String<LAllocator>& left, const ConstString&                                    right) { return left.Length() == right.Length() && !left.Compare(right); }
template<typename LAllocator, u64 ss_capacity, u64 ss_aligned_capacity> REV_INLINE bool operator==(const String<LAllocator>& left, const StaticString<ss_capacity, ss_aligned_capacity>& right) { return left.Length() == right.Length() && !left.Compare(right); }
template<typename LAllocator, typename RAllocator>                      REV_INLINE bool operator==(const String<LAllocator>& left, const String<RAllocator>&                             right) { return left.Length() == right.Length() && !left.Compare(right); }

template<typename RAllocator>                                           REV_INLINE bool operator==(const char                                           *left, const String<RAllocator>& right) { return strlen(left)  == right.Length() && !right.Compare(left); }
template<typename RAllocator>                                           REV_INLINE bool operator==(const ConstString&                                    left, const String<RAllocator>& right) { return left.Length() == right.Length() && !right.Compare(left); }
template<typename RAllocator, u64 ss_capacity, u64 ss_aligned_capacity> REV_INLINE bool operator==(const StaticString<ss_capacity, ss_aligned_capacity>& left, const String<RAllocator>& right) { return left.Length() == right.Length() && !right.Compare(left); }
...
template<typename LAllocator>                                           REV_INLINE bool operator<(const String<LAllocator>& left, const char                                           *right) { return left.Length() <= strlen(right)  && left.Compare(right) < 0; }
template<typename LAllocator>                                           REV_INLINE bool operator<(const String<LAllocator>& left, const ConstString&                                    right) { return left.Length() <= right.Length() && left.Compare(right) < 0; }
template<typename LAllocator, u64 ss_capacity, u64 ss_aligned_capacity> REV_INLINE bool operator<(const String<LAllocator>& left, const StaticString<ss_capacity, ss_aligned_capacity>& right) { return left.Length() <= right.Length() && left.Compare(right) < 0; }
template<typename LAllocator, typename RAllocator>                      REV_INLINE bool operator<(const String<LAllocator>& left, const String<RAllocator>&                             right) { return left.Length() <= right.Length() && left.Compare(right) < 0; }

template<typename RAllocator>                                           REV_INLINE bool operator<(const char                                           *left, const String<RAllocator>& right) { return strlen(left)  <= right.Length() && right.Compare(left) > 0; }
template<typename RAllocator>                                           REV_INLINE bool operator<(const ConstString&                                    left, const String<RAllocator>& right) { return left.Length() <= right.Length() && right.Compare(left) > 0; }
template<typename RAllocator, u64 ss_capacity, u64 ss_aligned_capacity> REV_INLINE bool operator<(const StaticString<ss_capacity, ss_aligned_capacity>& left, const String<RAllocator>& right) { return left.Length() <= right.Length() && right.Compare(left) > 0; }
...
template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(const String<Allocator>& left,       char                                            right) { return String<Allocator>::Concat(left, right); }
template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(const String<Allocator>& left, const char                                           *right) { return String<Allocator>::Concat(left, right); }
template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(const String<Allocator>& left, const ConstString&                                    right) { return String<Allocator>::Concat(left, right); }
template<typename Allocator, u64 ss_capacity, u64 ss_aligned_capacity> REV_INLINE String<Allocator> operator+(const String<Allocator>& left, const StaticString<ss_capacity, ss_aligned_capacity>& right) { return String<Allocator>::Concat(left, right); }
template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(const String<Allocator>& left, const String<Allocator>&                              right) { return String<Allocator>::Concat(left, right); }

template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(      char                                            left, const String<Allocator>& right) { return String<Allocator>::Concat(left, right); }
template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(const char                                           *left, const String<Allocator>& right) { return String<Allocator>::Concat(left, right); }
template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(const ConstString&                                    left, const String<Allocator>& right) { return String<Allocator>::Concat(left, right); }
template<typename Allocator, u64 ss_capacity, u64 ss_aligned_capacity> REV_INLINE String<Allocator> operator+(const StaticString<ss_capacity, ss_aligned_capacity>& left, const String<Allocator>& right) { return String<Allocator>::Concat(left, right); }

template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(String<Allocator>&& left,       char                                            right) { return String<Allocator>::Concat(RTTI::move(left), right); }
template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(String<Allocator>&& left, const char                                           *right) { return String<Allocator>::Concat(RTTI::move(left), right); }
template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(String<Allocator>&& left, const ConstString&                                    right) { return String<Allocator>::Concat(RTTI::move(left), right); }
template<typename Allocator, u64 ss_capacity, u64 ss_aligned_capacity> REV_INLINE String<Allocator> operator+(String<Allocator>&& left, const StaticString<ss_capacity, ss_aligned_capacity>& right) { return String<Allocator>::Concat(RTTI::move(left), right); }
template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(String<Allocator>&& left, const String<Allocator>&                              right) { return String<Allocator>::Concat(RTTI::move(left), right); }

template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(      char                                            left, String<Allocator>&& right) { return String<Allocator>::Concat(left, RTTI::move(right)); }
template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(const char                                           *left, String<Allocator>&& right) { return String<Allocator>::Concat(left, RTTI::move(right)); }
template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(const ConstString&                                    left, String<Allocator>&& right) { return String<Allocator>::Concat(left, RTTI::move(right)); }
template<typename Allocator, u64 ss_capacity, u64 ss_aligned_capacity> REV_INLINE String<Allocator> operator+(const StaticString<ss_capacity, ss_aligned_capacity>& left, String<Allocator>&& right) { return String<Allocator>::Concat(left, RTTI::move(right)); }
template<typename Allocator>                                           REV_INLINE String<Allocator> operator+(const String<Allocator>&                              left, String<Allocator>&& right) { return String<Allocator>::Concat(left, RTTI::move(right)); }
