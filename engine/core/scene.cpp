...
{
    CheckM(!s_SceneManager, "Scene manager is already created. Use SceneManager::Get() function instead");
...
{
    CheckM(s_SceneManager, "Scene manager is not created yet");
    return s_SceneManager;
}
...
{
    CheckM(m_Scenes.base->scene <= scene && scene < (m_Scenes.base + m_Scenes.size)->scene,
