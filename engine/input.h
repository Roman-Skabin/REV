//
// Copyright 2020 Roman Skabin
//

#pragma once

#include "core/core.h"
#include "math/vec.h"

typedef enum KEY
{
    KEY_FIRST       = 0x08,

    KEY_ESC         = 0x1B,
    KEY_TAB         = 0x09,
    KEY_BACK        = 0x08,
    KEY_CLEAR       = 0x0C,
    KEY_ENTER       = 0x0D,
    KEY_SPACE       = 0x20,

    KEY_LCTRL       = 0xA2,
    KEY_LSHIFT      = 0xA0,
    KEY_LALT        = 0xA4,

    KEY_RCTRL       = 0xA3,
    KEY_RSHIFT      = 0xA1,
    KEY_RALT        = 0xA5,
    
    KEY_CTRL        = 0x11,
    KEY_ALT         = 0x12,
    KEY_SHIFT       = 0x10,

    KEY_PRINT       = 0x2A,
    KEY_SNAPSHOT    = 0x2C,

    KEY_CAPSLOCK    = 0x14,
    KEY_NUMLOCK     = 0x90,
    KEY_SCROLL      = 0x91,
    
    KEY_HOME        = 0x24,
    KEY_END         = 0x23,
    KEY_INSERT      = 0x2D,
    KEY_DELETE      = 0x2E,
    
    KEY_LEFT        = 0x25,
    KEY_UP          = 0x26,
    KEY_RIGHT       = 0x27,
    KEY_DOWN        = 0x28,

    KEY_LWIN        = 0x5B,
    KEY_RWIN        = 0x5C,

    KEY_0           = 0x30,
    KEY_1           = 0x31,
    KEY_2           = 0x32,
    KEY_3           = 0x33,
    KEY_4           = 0x34,
    KEY_5           = 0x35,
    KEY_6           = 0x36,
    KEY_7           = 0x37,
    KEY_8           = 0x38,
    KEY_9           = 0x39,

    KEY_A           = 0x41,
    KEY_B           = 0x42,
    KEY_C           = 0x43,
    KEY_D           = 0x44,
    KEY_E           = 0x45,
    KEY_F           = 0x46,
    KEY_G           = 0x47,
    KEY_H           = 0x48,
    KEY_I           = 0x49,
    KEY_J           = 0x4A,
    KEY_K           = 0x4B,
    KEY_L           = 0x4C,
    KEY_M           = 0x4D,
    KEY_N           = 0x4E,
    KEY_O           = 0x4F,
    KEY_P           = 0x50,
    KEY_Q           = 0x51,
    KEY_R           = 0x52,
    KEY_S           = 0x53,
    KEY_T           = 0x54,
    KEY_U           = 0x55,
    KEY_V           = 0x56,
    KEY_W           = 0x57,
    KEY_X           = 0x58,
    KEY_Y           = 0x59,
    KEY_Z           = 0x5A,

    KEY_NUM_0       = 0x60,
    KEY_NUM_1       = 0x61,
    KEY_NUM_2       = 0x62,
    KEY_NUM_3       = 0x63,
    KEY_NUM_4       = 0x64,
    KEY_NUM_5       = 0x65,
    KEY_NUM_6       = 0x66,
    KEY_NUM_7       = 0x67,
    KEY_NUM_8       = 0x68,
    KEY_NUM_9       = 0x69,
    KEY_NUM_ADD     = 0x6B,
    KEY_NUM_SUB     = 0x6D,
    KEY_NUM_MUL     = 0x6A,
    KEY_NUM_DIV     = 0x6F,
    KEY_NUM_DECIMAL = 0x6E,
    KEY_NUM_SLASH   = 0x6C,

    KEY_F1          = 0x70,
    KEY_F2          = 0x71,
    KEY_F3          = 0x72,
    KEY_F4          = 0x73,
    KEY_F5          = 0x74,
    KEY_F6          = 0x75,
    KEY_F7          = 0x76,
    KEY_F8          = 0x77,
    KEY_F9          = 0x78,
    KEY_F10         = 0x79,
    KEY_F11         = 0x7A,
    KEY_F12         = 0x7B,
    KEY_F13         = 0x7C,
    KEY_F14         = 0x7D,
    KEY_F15         = 0x7E,
    KEY_F16         = 0x7F,
    KEY_F17         = 0x80,
    KEY_F18         = 0x81,
    KEY_F19         = 0x82,
    KEY_F20         = 0x83,
    KEY_F21         = 0x84,
    KEY_F22         = 0x85,
    KEY_F23         = 0x86,
    KEY_F24         = 0x87,

    KEY_PLUS        = 0xBB,
    KEY_MINUS       = 0xBD,

    KEY_DOT         = 0xBE,
    KEY_COMMA       = 0xBC,

    KEY_OEM_1       = 0xBA, // [;:] for US
    KEY_OEM_2       = 0xBF, // [/?] for US
    KEY_OEM_3       = 0xC0, // [`~] for US
    KEY_OEM_4       = 0xDB, // [[{] for US
    KEY_OEM_5       = 0xDC, // [\|] for US
    KEY_OEM_6       = 0xDD, // []}] for US
    KEY_OEM_7       = 0xDE, // ['"] for US
    KEY_OEM_8       = 0xDF,
    KEY_OEM_102     = 0xE2, // [<>] or [\|] on RT 102-key kbd.

    KEY_LAST        = 0xFF,
    KEY_MAX
} KEY;

typedef struct DigitalButton
{
    b32 down;
    b32 pressed;
    b32 released;
} DigitalButton, Key;

typedef struct Mouse
{
    v2s           pos;
    v2s           dpos;
    s32           wheel;
    s32           dwheel;
    DigitalButton left_button;
    DigitalButton middle_button;
    DigitalButton right_button;
    DigitalButton x1_button;
    DigitalButton x2_button;
} Mouse;

typedef struct AnalogButton
{
    f32 value;
    f32 threshold;
    b32 down;
    b32 pressed;
    b32 released;
} AnalogButton;

typedef struct Stick
{
    f32           deadzone;
    v2            offset;
    DigitalButton button;
} Stick;

typedef struct Gamepad
{
    DigitalButton button_a;
    DigitalButton button_b;
    DigitalButton button_x;
    DigitalButton button_y;
    AnalogButton  left_trigger;
    AnalogButton  right_trigger;
    DigitalButton left_shoulder;
    DigitalButton right_shoulder;
    DigitalButton button_up;
    DigitalButton button_down;
    DigitalButton button_left;
    DigitalButton button_right;
    Stick         left_stick;
    Stick         right_stick;
    DigitalButton button_start;
    DigitalButton button_back;
    b32           connected;
} Gamepad;
